...
Over the past 10-15 years, archaeologists have embraced, at least in theory, the advent of open source software development. There has been a lot of optimism regarding the potential value that archaeological software development would bring (particularly in the 2012 special issue of World Archaeology), however depending on who you talk to, creative use of computers among archaeologists has either been a perpetual novelty or a normalized way of working.

In this paper, we survey the field of archaeological software development in order to understand how archaeologists are _actually_ creating code. We rely rely on a data set called open-archaeo, which is a comprehensive list of software and resources developed by archaeologists for archaeological purposes. This is supplemented by data collected by GitHub, a collaborative open source software development platform that hosts the vast majority of projects listed in open-archaeo, and which automatically tracks users' contributions to various projects.

The metrics we use reflect how work is distributed across time and delegated among members of a project and among the community as a whole. These data are analyzed to highlight how collaborative software development operates within our community.

This aspect of _collaborative_ software development is crucial to our interest in the historical development of software engineering among archaeologists. Archaeological software development is often (or nearly always) discussed as _open source_ software development. A technical definition of open source software is software whose source code is publicly accessible, and which is free to be used, modified or built upon by anyone and for any purpose. This implies, but does not necessarily require or benefit from, various collaborative endeavours. For instance, code written by a single person that is distributed under an open license, but which is never accessed or reused at any point after it is made available, meets this technical definition of open source, but misses out on the potential value that may be contributed by others. A revised definition of open source software defines it as software developed in a collaborative manner, as code that belongs to the community of developers, users and stakeholders who find value in the product and strive for its continued development and iterative improvement. We do not deny solitary software developers as members of a broader community; however their behaviour as solitary developers may imply the existence of widespread norms that encourage more basic, but perhaps less fulfilling, ways of working.

In this sense, our study is about the development of norms and practices throughout the archaeological software development community. We have our own unique flavour, based on our distinct goals, constraints, mindsets and modes of organizing ourselves as archaeologists, which shape how we write code. Our goal in this paper is to highlight how our archaeological open source culture has emerged and continues to develop.
...
#### Categories, tags, platforms and hosts
...
ZB: Break down platform by contributors, since a few people may be prolific in their support of a platform without necessarily representing the whole population.
...
The vast majority of projects included in open-archaeo are hosted on GitHub, but a few other hosts are used too. If we break this down by number of unique project maintainers, GitHub still remains dominant, but we see that each other platform is only used by a handful of individuals.
...
ZB: Plot host by number of unique users.
...
ZB: Lifespan by tag is dominated by ancient DNA tags. Another hint that it may be best to lump all of those. In general though, I'm not sure how helpful the following tables are, unless we account for volume somehow.


```{r plot-lifespan-activity-tags}
#TODO: think we might need some exclusion rules here 
...
ZB: Add a chart, limited to repos with more than 1 contributor. It should present how evenly distributed commits are across all contributors. So maybe a percent stacked bar chart, with bars representing a repository, with each bar broken down into sections representing each inidividual contributor, and each section's size representing the percentage of the individual's commits out of the total from the repo.
